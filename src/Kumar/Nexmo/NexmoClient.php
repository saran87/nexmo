...
class NexmoClient
{
     ...
     * unicode characters.
     */
    function sendText($to, $message, $unicode = null)
    {

        // Making sure strings are UTF-8 encoded
        if (!is_numeric($this->from) && !mb_check_encoding($this->from, 'UTF-8')) {
        ...
        $from = $this->validateOriginator($this->from);
    ...
    private function nexmoParse($from_nexmo)
    {
        ...
        if ($response_obj) {
            ...
            $response_obj = (array) $response_obj;
            $total_cost = 0;
            ...
            if (is_array($response_obj['messages'])) {
               ...
               return $response_obj['messages'][0];
            }
